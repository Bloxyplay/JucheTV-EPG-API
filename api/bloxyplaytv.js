...
export default async function handler(req, res) {
  // CORS headers, including the custom auth headers — required for the browser
  // preflight (OPTIONS) to succeed when a request carries X-KORYO-TV/X-JUCHE-TV.
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-KORYO-TV, X-JUCHE-TV');
  res.setHeader('Cache-Control', 'no-store');

  // Preflight — answer immediately, no auth/logic needed for OPTIONS.
